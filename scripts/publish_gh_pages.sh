...
if [ -f .gitignore ] && [ -d docs ]; then
  ...
  if [ -f .gitignore ] && [ -d docs ]; then
    if [ ! -d docs ]; then
      ...
      npm prune || {
        echo 'Error: Failed to prune npm packages' >&2
        exit 1
      } || {
        echo 'Error: Failed to prune npm packages' >&2
        exit 1
    ...
    if [ -d docs ]; then
      if [ -d docs ]; then
        git subtree split --prefix docs -b gh-pages || {
          echo 'Error: Failed to create subtree split' >&2
          exit 1
        } || exit 1
      else
        echo 'Error: Directory docs does not exist' >&2
    ...
    git push -f origin gh-pages:gh-pages || {
      echo 'Error: Failed to force push gh-pages branch to origin' >&2
      exit 1
    } # Force push 'gh-pages' branch to origin
  else
